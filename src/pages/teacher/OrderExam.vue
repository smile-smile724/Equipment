<template>
  ...
  <div id="mainbox">
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right" style="margin:0 0 15px 40px;font-size:18px;">
      ...
      <el-breadcrumb-item>设备预约审核</el-breadcrumb-item>
    </el-breadcrumb>
      ...
      || data.time.toLowerCase().includes(search.toLowerCase())).slice((currentPage-1)*pagesize,currentPage*pagesize)">
        ...
        align="center">
          <template slot-scope="scope">
            ...
            @click="Approval(scope.row)">同意申请</span>
          </template>
        </el-table-column>
        <el-table-column
        label="拒绝"
        align="center">
          <template slot-scope="scope">
            ...
            @click="Refuse(scope.row)">拒绝申请</span>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
...
export default {
    ...
    methods: {
      ...
      handleDelete(row,pass){
          const that=this;
          axios({
            method: 'post',
            url: this.$host+'/borrowing/appointment/examine',
            params:{
